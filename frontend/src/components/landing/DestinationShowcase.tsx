    ...
    <section id="destinations" className="py-20 lg:py-28 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
      ...
      <FadeIn className="space-y-3 max-w-2xl">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-50 text-[#F95724] text-xs font-bold uppercase tracking-wider">
          <MapPin className="w-3.5 h-3.5" /> Destination Discovery
        </div>
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight">
          Places that deserve a place on your map.
        </h2>
        <p className="text-sm sm:text-base text-slate-500 leading-relaxed">
          From historic shrines to sun-drenched coastal cliffs, discover curated destinations crafted for your itinerary.
        </p>
      </FadeIn>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-6 items-stretch">
        {LANDING_DESTINATIONS.map((dest, idx) => {
          // Asymmetric column spans: 1st and 4th items are wider editorial cards
          ...
          return (
            ...
            >
              <div className="group relative h-80 sm:h-96 w-full rounded-3xl overflow-hidden border border-slate-200/90 shadow-xs hover:shadow-xl transition-all duration-300 flex flex-col justify-end p-6 sm:p-8">
                ...
                <div className="absolute inset-0 overflow-hidden">
                  <img
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/40 to-black/10 group-hover:bg-slate-950/80 transition-colors duration-300" />
                </div>

                {/* Top Floating Badge */}
                <div className="absolute top-5 left-5 right-5 flex items-center justify-between z-10">
                  <span className="px-3 py-1 rounded-full text-xs font-bold bg-white/20 backdrop-blur-md text-white border border-white/20 shadow-xs">
                    {dest.costLevel}
                  </span>

                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-black/40 backdrop-blur-md text-white border border-white/10">
                    <Star className="w-3 h-3 text-amber-400 fill-amber-400" />
                    {dest.rating}
                  </span>
                </div>

                {/* Bottom Content with Micro-interaction Lift */}
                <div className="relative z-10 space-y-2.5 text-white transform group-hover:-translate-y-1 transition-transform duration-300">
                  <div className="space-y-1">
                    <div className="flex items-baseline gap-2">
                      <h3 className="text-2xl sm:text-3xl font-extrabold uppercase tracking-tight text-white group-hover:text-orange-400 transition-colors">
                        {dest.name}
                      </h3>
